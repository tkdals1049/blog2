...
function loadSkills(skills){
	...
	while(i<skills.length){
		...
		for(i=0;i<skills.length;i++){
			var skill = '<div class="col m2"><svg viewBox="0 0 120 120"><path d="'+skills[i].icon+'"></path></svg>'+skills[i].name+'</div>';
			row+=skill;
		}
		row+='</div>';
		skillsInnerHTML+=row;
...
function onWindowResize(){
	...
	if (tabs && tabs[0]) {
		tabs[0].style.width = parseInt($('#pageb').css('width').replace('px', ''), 10)-15;
...
function(data, status){
	...
	$('#tabs').html(`					
		<li class="tab col s1"><a href="#hello">Hello</a></li>
		<li class="tab col s1"><a href="#skills">Skill</a></li>
		<li class="tab col s2"><a href="#projects">Projects</a></li>
		<li class="tab col s2"><a href="#experience">Experience</a></li>
		<li class="tab col s1"><a href="#board">Borad</a></li>
	`);
